...
const INDIAN_CITIES = [
  ...
  'Ulhasnagar, Maharashtra', 'Nellore, Andhra Pradesh', 'Jammu, Jammu and Kashmir', 'Sangli-Miraj & Kupwad, Maharashtra',
  ...
  'Kurnool, Andhra Pradesh', 'Rajpur Sonarpur, West Bengal', 'Rajahmundry, Andhra Pradesh', 'Bokaro, Jharkhand',
  'South Dumdum, West Bengal', 'Bellary, Karnataka', 'Patiala, Punjab', 'Gopalpur, Odisha',
  'Agartala, Tripura', 'Bhagalpur, Bihar', 'Muzaffarnagar, Uttar Pradesh', 'Bhatpara, West Bengal',
  'Panihati, West Bengal', 'Latur, Maharashtra', 'Dhule, Maharashtra', 'Rohtak, Haryana',
  'Korba, Chhattisgarh', 'Bhilwara, Rajasthan', 'Berhampur, Odisha', 'Muzaffarpur, Bihar',
  'Ahmednagar, Maharashtra', 'Mathura, Uttar Pradesh', 'Kollam, Kerala', 'Avadi, Tamil Nadu',
  'Kadapa, Andhra Pradesh', 'Kamarhati, West Bengal', 'Sambalpur, Odisha', 'Bilaspur, Chhattisgarh',
  'Shahjahanpur, Uttar Pradesh', 'Satara, Maharashtra', 'Bijapur, Karnataka', 'Rampur, Uttar Pradesh',
  'Shivamogga, Karnataka', 'Chandrapur, Maharashtra', 'Junagadh, Gujarat', 'Thrissur, Kerala',
  'Alwar, Rajasthan', 'Bardhaman, West Bengal', 'Kulti, West Bengal', 'Kakinada, Andhra Pradesh',
  'Nizamabad, Telangana', 'Parbhani, Maharashtra', 'Tumkur, Karnataka', 'Khammam, Telangana',
  'Ozhukarai, Puducherry', 'Bihar Sharif, Bihar', 'Panipat, Haryana', 'Darbhanga, Bihar',
...
export default function EditRideScreen({ navigation, route }: any) {
  const { user } = useAuth();
  ...
  const loadRideData = async () => {
    try {
      const { data, error } = await supabase
        ...
        .select('*')
        .eq('id', rideId)
        .eq('organizer_id', user?.id) // Ensure user can only edit their own rides
        .single();
      ...
      if (data) {
        ...
        setDifficulty(data.difficulty_level || 'moderate');
        
        // Parse existing duration if it exists
        if (data.estimated_duration_hours) {
          const totalHours = parseFloat(data.estimated_duration_hours);
  ...
  const handleUpdateRide = async () => {
    ...
    if (!user) {
      Alert.alert('Error', 'You must be logged in to update rides');
      return;
    }
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update(updateData)
        .eq('id', rideId)
        .eq('organizer_id', user.id); // Ensure user can only update their own rides
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <TouchableOpacity 
            ...
            style={[styles.headerButton, { opacity: updating ? 0.5 : 1 }]}
            disabled={updating}
          >
